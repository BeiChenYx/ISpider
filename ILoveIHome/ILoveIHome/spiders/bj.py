    ...
    def parse(self, response):
        ...
        for home in homes:
            try:
                ...
                item['covered'] = ' '.join([covered.strip() for covered in list(home.xpath(covered_xpath))])
                item['addr'] = ' '.join([addr.strip() for addr in list(home.xpath(addr_xpath))])
                item['time'] = ' '.join([time.strip() for time in list(home.xpath(time_xpath))])
                item['type_home'] = ' '.join([ty.strip() for ty in list(home.xpath(type_xpath))])
                item['price'] = ' '.join([price.strip() for price in list(home.xpath(price_xpath))])
